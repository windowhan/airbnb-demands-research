...
### Phase 1: 기반 구축 ✅ 완료
- [x] 프로젝트 구조 생성
- [x] 서울 지하철역 좌표 데이터 수집/정리 (`config/stations.json`)
- [x] SQLite DB 스키마 구축 (`models/schema.py`, `models/database.py`)
- [x] Airbnb API 클라이언트 기본 구현 (`crawler/airbnb_client.py`)

### Phase 2: 핵심 크롤러 ✅ 완료
- [x] 검색 크롤러 구현 (`crawler/search_crawler.py`)
- [x] 캘린더 크롤러 구현 (`crawler/calendar_crawler.py`)
- [x] 숙소 상세 크롤러 구현 (`crawler/listing_crawler.py`)
- [x] Rate limit / 에러 핸들링 (`crawler/rate_limiter.py`)
- [x] API 키 자동 추출 (`crawler/api_key_extractor.py`)
- [x] 프록시 매니저 (`crawler/proxy_manager.py`)

### Phase 3: 분석 로직 + 스케줄러 ✅ 완료
- [x] APScheduler 설정 (`scheduler/jobs.py`)
- [x] 예약률 계산 로직 (`analysis/booking_rate.py`)
- [x] 수익률 추정 로직 (`analysis/revenue.py`)
- [x] 지역별/숙소별 집계 (`analysis/aggregator.py`)

### Phase 4: 시각화 ✅ 완료
- [x] Streamlit 대시보드 기본 레이아웃 (`dashboard/app.py`)
- [x] 전체 현황 페이지 (`dashboard/pages/overview.py`)
- [x] 역별 상세 분석 페이지 (`dashboard/pages/station_detail.py`)
- [x] 숙소 유형별 분석 페이지 (`dashboard/pages/listing_type.py`)
- [x] 수익률 지도 페이지 (`dashboard/pages/revenue_map.py`)
- [x] 재사용 차트 컴포넌트 (`dashboard/components/charts.py`)
- [x] Jupyter Notebook 분석 템플릿 (4종)
...
| 파일 | 대상 모듈 |
|------|-----------|
| `test_airbnb_client.py` | API 클라이언트 |
| `test_api_key_extractor.py` | API 키 자동 추출 |
| `test_search_crawler.py` | 검색 크롤러 |
| `test_calendar_crawler.py` | 캘린더 크롤러 |
| `test_listing_crawler.py` | 숙소 상세 크롤러 |
| `test_rate_limiter.py` | Rate limiter |
| `test_proxy_manager.py` | 프록시 매니저 |
| `test_models.py` | DB 모델/스키마 |
| `test_booking_rate.py` | 예약률 계산 |
| `test_revenue.py` | 수익률 추정 |
| `test_aggregator.py` | 집계 로직 |
| `test_jobs.py` | 스케줄러 작업 |
| `test_dashboard_pages.py` | 대시보드 페이지 |
| `test_charts.py` | 차트 컴포넌트 |
| `test_main.py` | CLI 진입점 |
| `test_settings.py` | 설정값 |
